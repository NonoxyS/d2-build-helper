...
| `:shared:common` | KMP | `coRunCatching`, `ResultExtensions`, `OneTimeEvent`, `TimeConverter`, `Mapper`, `CoroutineDispatchers`, `commonModule` Koin. `iosMain/utils/FileUtils.kt` exposes `getImageCacheDirectory()` (NSFileManager wrapper) for iOS Coil disk cache. |
| `:shared:common-ui` | KMP + Compose | Design system v1: `compose/theme/` (Theme/Colors/Typography/Shapes/Fonts — dark-only, KMPTemplate-style), `compose/components/shared/` (Space ramp, `D2LoadingView`, `D2ErrorView`, `D2TopBar`, `D2Dialog`, `setupImageLoader()`/`createImageLoader()` expect/actual for Coil singleton). |
| `:shared:common-resources` | KMP + moko-resources | Shared `strings.xml`, `colors.xml` (DS tokens), fonts (NotoSans Regular/Medium/Bold), and position/Radiant/Dire chrome images under `moko-resources/{base, fonts, images}/`. `MR` accessor lives in package `dev.nonoxy.d2buildhelper.common.resources`. |
...
| `mobile-resources.mdc` | `:shared:common-resources` (moko-resources 0.26.x — `MR.strings/fonts/images/colors`), design-system tokens |
...
- Design system: dark-only Stratz-tuned palette in `:shared:common-resources/.../moko-resources/base/colors.xml` (`background`, `surface`, `surfaceVariant`, `tintColor`, `textPrimary`, `textSecondary`, `outline`). Theme entry is `D2BuildHelperTheme { ... }` from `dev.nonoxy.d2buildhelper.common.ui.compose.theme`; tokens are accessed via `D2BuildHelperTheme.colors/typography/shapes` (read-only composables, populated by the surrounding theme). Typography is `captionMD/textMD/bodyLG/headlineMD` on `fontNotoSans`. Shapes are `cornerRadius4/8/12/16`. Shared utilities live under `compose/components/shared/{space,loading,error,topbar,dialog,image}` — each composable takes rich parameters with `D2BuildHelperTheme.*` defaults, so callers customize only what they need. Spacings use scope-aware `SpaceN()` (Tailwind ramp 2/4/6/8/12/16/20/24/32/40/48) instead of inline `Spacer(Modifier.width/height(Xdp))`.
- Coil image loader is configured once per platform via `setupImageLoader()` (called after `initKoin` in `AndroidApp.onCreate` and the iOS `MainViewController` configure block). Both platforms ship a 64MB memory cache and a 256MB disk cache. The iOS implementation uses `getImageCacheDirectory()` from `:shared:common/iosMain/utils/FileUtils.kt`. AsyncImage call sites never pass `imageLoader` explicitly — the singleton resolves implicitly.
