...
internal fun TalentTreeIcon(
    ...
    val talentEmpty = D2BuildHelperTheme.colors.talentEmpty
    val talentGold = D2BuildHelperTheme.colors.talentGold
    Canvas(modifier = modifier.size(width, height)) {
        ...
        TIER_Y_FRACTIONS.forEachIndexed { index, yFraction ->
            ...
            drawLine(
                color = branchColor(model, TalentSide.LEFT, talentEmpty, talentGold),
            ...
            drawLine(
                color = branchColor(model, TalentSide.RIGHT, talentEmpty, talentGold),
                start = Offset(trunkX, y),
                end = Offset(trunkX + branchDx, y - branchDy),
                strokeWidth = size.width * BRANCH_STROKE_FRACTION,
                cap = StrokeCap.Round,
...
private fun branchColor(model: UiTalentTier?, side: TalentSide, talentEmpty: Color, talentGold: Color) = when {
    model == null || !model.taken -> talentEmpty
    model.side == side -> talentGold
    model.side == null -> talentGold.copy(alpha = TAKEN_UNKNOWN_ALPHA) // talent absent from hero constants → dim both
    else -> talentEmpty
}
